...
That line is the ONLY source for `cloudId` and the project key. No line → the repo has no Jira wiring: skip every Jira step (steps 2 and 8's transition), ship the PR anyway, and say so in the report. Never guess a cloudId, never carry one over from another project — a wrong cloudId transitions some other project's ticket.

Fallbacks when the line is absent but the user names a key: `mcp__atlassian__getAccessibleAtlassianResources` to resolve the cloudId. Still ambiguous → ask; do not pick one.

**Transition ids are per-project and change** — always `getTransitionsForJiraIssue` and match on `to.name`, never a hardcoded id. Same for the browse URL: derive it from the site the cloudId resolves to, not a literal host.
...
5. **Write the commit message — from what is STAGED, not from the plan alone.**
   ...
   Then commit. Conventional Commits, subject ≤50 chars, body explains the WHY. **Prefix the subject with the Jira key when one applies** so the GitHub-for-Jira app auto-links it: `<KEY>-12 feat(trading): strong bias only`. End with:
   ```
...
7. **Push + open PR.**
   ...
   **No AI attribution footer in the PR body** (owner rule 2026-07-08: repos go to external
   devs — body is what/why/safety + Jira ref only, nothing else).
   **Jira ref = clickable link** (owner rule 2026-07-08): end the body with
   ...
   app, link in the body for humans. Derive `<site-url>` from the resolved cloudId.

8. **Report** the PR URL, plus the receipt verdict from step 0 (tests green on attempt N, review clean / N warnings shipped-anyway / receipt gate overridden). STOP — do not merge. If the work has an issue and the Atlassian MCP is authenticated, move it to **In Review** (`getTransitionsForJiraIssue` → the transition whose `to.name == "In Review"`; `transitionJiraIssue` with the cloudId from CLAUDE.md). Transition ids differ per project — match on name, never hardcode an id. No `In Review` transition in this project's workflow → log it and move on.

   The move to **Done** happens at MERGE time, not here — `/ship` only opens the PR (the owner merges). Do the Done transition after merging: `getTransitionsForJiraIssue` → match `to.name == "Done"` → `transitionJiraIssue`. Do NOT rely on the native GitHub-for-Jira "PR merged → Done" rule (verified flaky). Nothing automates this today — `/ship-and-deploy` used to claim it and was archived 2026-07-21 without ever having wired it.
